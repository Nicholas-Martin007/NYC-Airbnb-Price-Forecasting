...
from model import concat_data, createAndSaveScaler, createDecisionTree, createLinearRegression, createSVR, find_optimal_cluster, loadScaler, one_hot_encoder
...
def train_pipeline(data, split_ratio=0.2):
    '''
    type data: pd.DataFrame
    type split_ratio: float
    rtype: None

    Split Data, Normalization, Clustering, Regression
    '''
    ...
    train_data, test_data = train_test_split(data, test_size=split_ratio, random_state=42, shuffle=True)

    # Normalization (Train_data)
    train_data['latitude'] = lat_scaler.transform(train_data[['latitude']])
    train_data['longitude'] = long_scaler.transform(train_data[['longitude']])
    # train_data['log_price'] = log_price_scaler.transform(train_data[['log_price']]) 
    ...
    train_data = train_data.drop(target_cluster[0], axis=1).drop(target_cluster[1], axis=1)
    test_data = test_data.drop(target_cluster[0], axis=1).drop(target_cluster[1], axis=1)
...
if __name__ == '__main__':
    ...
    train_pipeline(df, split_ratio=0.2)